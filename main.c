...
#if defined _BSD_SOURCE || defined _XOPEN_SOURCE && \((_XOPEN_SOURCE - 0) >= 500 || defined _XOPEN_SOURCE_EXTENDED)        
...
void remove_file(int n, bool manual)                                      
{
	...
	if (n + 1 < tns.cnt) 
	{
		memmove(tns.thumbs + n, tns.thumbs + n + 1, (tns.cnt - n - 1) * sizeof(thumb_t));    
