...
void check_add_file(char* filename)
{
	...
	if (filename == NULL || *filename == '\0')            //파일이름이 없거나 주소안에 아무것도 없을
		return;

	if (access(filename, R_OK) < 0) {                  //access : 파일 존재 조사, filename 경로, mode 유무조사 정상시 0, 에러시 -1                              
		warn("could not open file: %s", filename);         //파일이 없을임
		return;
	}

	if (fileidx == filecnt) {                        //fileidx가 filent일때 filecnt에 2를 곱하고
		filecnt *= 2;
	...
	((_XOPEN_SOURCE - 0) >= 500 || defined _XOPEN_SOURCE_EXTENDED)            //XOPEN_SOUCRE - 0 가 500보다 크거나 _XOPEN_SOURCE_EXTENDED 정의됫을경우
		...
		if ((files[fileidx].path = realpath(filename, NULL)) == NULL) {            //files[fileidx]의 path가 절대경로(filename.NULL)이고 이게 NULL이면 realpath가 아니라는 경고
			warn("could not get real path of file: %s\n", filename);
			return;
		}
#else
	if (*filename != '/') {                                          //filename의 주소값이 '/'이 아니면
		if ((files[fileidx].path = absolute_path(filename)) == NULL) {         //files[fileidx]의 path가 절대경로이고 이게 NULL이면 절대경로 아니라는 경고를 뛰움
...
void remove_file(int n, bool manual)                                 //파일 제거   ( bool : ture , false 저장         )               
{
	...
	if (n + 1 < filecnt)
		memmove(files + n, files + n + 1, (filecnt - n - 1) * sizeof(fileinfo_t));   //memove:(복사되는 메모리의 첫번 주소, 복사할 메모리의 첫번 주소,복사할크기)
	if (n + 1 < tns.cnt) {
		memmove(tns.thumbs + n, tns.thumbs + n + 1, (tns.cnt - n - 1) * sizeof(thumb_t));       //memove:(복사되는 메모리의 첫번 주소, 복사할 메모리의 첫번 주소,복사할크기)
...
void open_info(void)                                                //open 정보
{
	...
	if (info.cmd == NULL || info.open || win.bar.h == 0)                     //info라는 구조체 값이 (info.cmd == NULL || info.open || win.bar.h == 0)         
		return;
	if (info.fd != -1) {                                             //info.fd != -1이면 close(info.fd
	...
	if (pid > 0) {                                                   //parent 동작            
		...
		fcntl(pfd[0], F_SETFL, O_NONBLOCK);                                 //fcntl : (제어할 파일의  파일 기술자, 파일 기술자에 대한 특성을 제어하는 매개변수, 두번인자에 의해 결정되는 선택적인 값)
		info.fd = pfd[0];                                             //info.fd = pfd[0] 하고 , info.open을 참으로
		info.i = info.lastsep = 0;
		info.open = true;
	}
	else if (pid == 0) {                                             //children 동작
...
void load_image(int new)
{
	...
	while (!img_load(&img, &files[new])) {                        //!img_load실패시 파일삭제
...
void update_info(void)                                                      //update 정보
{
	...
	if (mode == MODE_THUMB) {                                                //thumb시 mark
		if (tns.cnt == filecnt) {
...
void reset_cursor(void)
{
	...
	if (mode == MODE_IMAGE) {                                    //이미지 모드에서 커서 리셋                     
		for (i = 0; i < ARRLEN(timeouts); i++) {
...
void animate(void)
{
	if (img_frame_animate(&img, false)) {                           //img에 frame 제작 생기를 불어넣는다
		redraw();
		set_timeout(animate, img.multi.frames[img.multi.sel].delay, true);
	}
}
...
void run_key_handler(const char* key, unsigned int mask)
{
	...
	if (keyhandler.cmd == NULL) {											    //keyhandler.cmd  없으면 경고
...
void run(void)																	//input값을 확인하고 그에 맞는 이벤트를 실행하는 함수, 모드에 맞는 이벤트 (keypass,커서)이런것 실행                     
{
	...
	while (true) {
		...
		case ButtonPress:														//버튼이 눌린경우 버튼press함수
			...
			break;
		case ClientMessage:														//다른 클라이언트가 메세지를 보내왔을         
			if ((Atom)ev.xclient.data.l[0] == atoms[ATOM_WM_DELETE_WINDOW])
				return;
			break;
		case ConfigureNotify:													//창의 상태가 바뀌었을                  
